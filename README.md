# Transformer Dynamics as Gradient Flow on $S^1$
...
`ascending=false` (gradient descent). In USA, setting
`self_attention=false` removes the $j=i$ term. In SA, self-interaction is always
included.
