...
def _save_frames_and_gif(
    ...
    is_null = a.size == 0 and omega.size == 0
    frame_suffix = label_slug
    if is_null and not frame_suffix.endswith("mlp0"):
        frame_suffix = f"{frame_suffix}_mlp0"
        ...
        if frame_pos in frame_labels:
            ...
            if frame_fig.axes:
                for ax in frame_fig.axes:
                    ax.set_title("")
            frame_path = frames_dir / f"frame_{label}_{frame_suffix}.pdf"
            ...
            if a.size:
                ...
                alt_path = frames_dir / f"frame_{label}_{frame_suffix}_{suffix}.pdf"
...
def _save_field_gif(
    ...
    for idx in frame_indices:
        ...
        if not ascending:
            # Match the simulated drift: negate the total field for descent.
            att = -att
            mlp = -mlp
        field = att + mlp
        fields.append(field)
    ...
    frame_iter = iter_progress(range(len(frame_times)), label=f"Field {label_slug}", unit="frame")
    for frame_idx in frame_iter:
        field = fields[frame_idx]
        att = att_fields[frame_idx]
        mlp = mlp_fields[frame_idx]
...
def run_experiment(config: RunConfig) -> None:
    ...
    for beta in config.betas:
        ...
        # Cap threshold by dimension-dependent rule: pi/(2d).
        ...
        for mlp_scale_eff, params_json in scale_runs:
            ...
            for i, mlp_params in enumerate(mlp_params_list):
                ...
                for j, theta_hist in enumerate(mlp_histories):
                    ...
                    plt.close(fig)
                    hist_stem = hist_dir / f"histogram{mlp_suffix}"
                    ...
                    plt.close(fig)
                    hist_null_stem = hist_dir / f"histogram_with_null{mlp_suffix}"
                    fig = make_histogram_figure(
                        theta_hist[-1],
                        null_histories[j][-1],
                        a=mlp_params.a,
        ...
        del null_histories, null_times, theta0_list
        null_histories = null_times = theta0_list = None
        import gc
        gc.collect()
